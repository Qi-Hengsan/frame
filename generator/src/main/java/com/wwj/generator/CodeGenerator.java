...
import com.wwj.generator.config.*;
import com.wwj.generator.config.query.GeneratorMySqlQuery;
import com.wwj.generator.config.query.GeneratorSqlServerQuery;
import com.wwj.generator.constant.GeneratorConstant;
import com.wwj.generator.exception.GeneratorException;
...
public class CodeGenerator {
    ...
    public void init(String tableName, GeneratorProperties generatorProperties) {
        ...
        if (StringUtils.isNotBlank(parentPackage)) {
            generatorProperties.setParentPackagePath(parentPackage.replaceAll(
    ...
    public void generator(String tableName, String pkIdName, GeneratorProperties generatorProperties) {
        ...
        MybatisGeneratorConfig mybatisGeneratorConfig = generatorProperties.getMybatisGeneratorConfig();
        if (mybatisGeneratorConfig == null) {
            throw new GeneratorException("mybatisPlusGeneratorConfig不能为空");
        }

        // 全局配置
        GlobalConfig globalConfig = mybatisGeneratorConfig.getGlobalConfig();
        ...
        if (DbType.MYSQL == dbType) {
            // 设置MySQL元数据自定义查询
            dataSourceConfig.setDbQuery(new GeneratorMySqlQuery());
        } else if (DbType.SQL_SERVER2005 == dbType || DbType.SQL_SERVER == dbType) {
            // 设置SQLServer元数据自定义查询
            dataSourceConfig.setDbQuery(new GeneratorSqlServerQuery());
        }
        ...
        // 包配置
        PackageConfig packageConfig = mybatisGeneratorConfig.getPackageConfig();
        ...
        // 策略配置
        StrategyConfig strategyConfig = mybatisGeneratorConfig.getStrategyConfig();
        ...
        InjectionConfig injectionConfig = new InjectionConfig() {
            ...
            public void initMap() {
                ...
                TableInfo tableInfo = this.getConfig().getTableInfoList().stream().filter(
                        it->Objects.equals(tableName,it.getName()))
                        .findAny().get();
                ...
                map.put("PPC", pageParamPackage + StringPool.DOT + entityName + GeneratorConstant.PAGE_PARAM);
                // 查询参数共公包路径
               ...
               map.put("QVOC",queryVOPackage + StringPool.DOT + entityName + GeneratorConstant.QUERY_VO);
                // 响应结果类路径
